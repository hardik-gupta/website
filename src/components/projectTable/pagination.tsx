    ...
    <div className="flex items-center justify-between w-full">
      <div className="flex items-center space-x-2 lg:space-x-4">
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 p-1 flex gap-2">
              <SelectValue placeholder={table.getState().pagination.pageSize} className="mr-1" />
            </SelectTrigger>
            <SelectContent side="bottom" className="bg-white min-w-[30px] text-black">
              {[10, 20, 30, 40, 50].map((pageSize) => (
                <SelectItem key={pageSize} value={`${pageSize}`} className="rounded-lg text-black">
                  {pageSize}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="flex items-center space-x-1">
          ...
          >
            ...
            <MdKeyboardArrowLeft className="h-8 w-8" fill={!table.getCanPreviousPage() ? '#cfcfcf' : 'black'} />
          </button>
          <button
            className="h-5 h-5 md:h-8 md:w-8 p-0 flex items-center justify-center"
          ...
          >
            ...
            <MdKeyboardArrowRight className="h-8 w-8" fill={!table.getCanNextPage() ? '#cfcfcf' : 'black'} />
          </button>
          <button
            className="hidden h-8 w-8 p-0 lg:flex items-center justify-center"
